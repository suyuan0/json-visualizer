...
import { isIframe } from "@/lib/helpers";
import { useConfig } from "./useConfig";
import { useJson } from "./useJson";
import { contentToJson, jsonToContent } from "@/lib/utils";
...
const defaultJson = JSON.stringify(
  {
    ...
    registration_date: "2024-11-15T08:00:00Z"
  },
  null,
  2
);
...
const debouncedUpdateJson = _.debounce((value: unknown) => {
  useGraph.getState().setLoading(true);
  useJson.getState().setJson(JSON.stringify(value, null, 2));
}, 800);

export const useFile = create<FileStates & JsonActions>()((set, get) => ({
  ...initialStates,
  clear: () => {
    set({ contents: "" });
